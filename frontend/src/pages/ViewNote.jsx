...
const ViewNote = () => {
  ...
  // handle user note update function
  const handlUpdateNote = async (e) => {
    e.preventDefault()
    try {
      // send data
      await axios.patch("https://us-central1-notetakingapi.cloudfunctions.net/app/api/note/update", {
        id: id,
        title: form.title,
  ...
  // handle user note delete function
  const handleNoteDelete = async (userid) => {
    try {
      // send delete data
      await axios.post("https://us-central1-notetakingapi.cloudfunctions.net/app/api/note/delete", { id: userid })
        .then((res) => {
          console.log(res)
          if (res.data.status === "success") {
            // alert
